"""
Dataset for Attention-based Stock Price Movement Prediction
============================================================
Same as LSTM: Aggregates news per DAY, creating a sequence of daily embeddings.
...
def get_daily_news_sequence(
    ...
    """
    Get sequence of daily aggregated news embeddings with mask.
    
    Returns:
        embeddings: Numpy array of shape (window_days, embedding_dim)
        mask: Numpy array of shape (window_days,) - 1 for days with news, 0 for empty
    """
    ...
    mask = np.zeros(window_days, dtype=np.float32)
...
def create_combined_dataset(
    ...
    verbose: bool = True
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str], List[pd.Timestamp]]:
    """
    Create combined dataset with stock embeddings and daily news sequences.
    
    Returns:
        Tuple of (stock_embeddings, news_sequences, news_masks, labels, ticker_indices, ticker_names, dates)
        - stock_embeddings: [N, stock_embedding_dim]
        - news_sequences: [N, window_size, news_embedding_dim]
        - news_masks: [N, window_size] - 1 where news exists, 0 otherwise
        - labels: [N]
        - ticker_indices: [N]
    """
    if tickers is None:
    ...
    for ticker in tickers:
        ...
        try:
            ...
            for idx, row in stock_emb_df.iterrows():
                ...
                # Get DAILY news sequence WITH MASK - using pre-grouped news
                news_sequence, news_mask = get_daily_news_sequence(news_df, window_end_date, news_by_date=news_by_date)
                ...
                all_news_masks.append(news_mask)
    ...
    news_masks = np.array(all_news_masks, dtype=np.float32)
    technical_features = np.array(all_technical_features, dtype=np.float32)
    labels = np.array(all_labels, dtype=np.int64)
    ticker_indices = np.array(all_ticker_indices, dtype=np.int64)
    
    if verbose:
        avg_news_days = news_masks.sum(axis=1).mean()
        print(f"\n{'='*60}")
        print(f"DATASET SUMMARY (Attention Strategy)")
        print(f"{'='*60}")
        ...
        print(f"  Average days with news: {avg_news_days:.1f}/{Config.WINDOW_SIZE}")
        print(f"\n  Class distribution:")
        for i, name in enumerate(Config.CLASS_NAMES):
            count = (labels == i).sum()
            print(f"    {name}: {count} ({count/len(labels)*100:.1f}%)")
    
    return stock_embeddings, news_sequences, news_masks, technical_features, labels, ticker_indices, tickers, all_dates


class AttentionPredictionDataset(Dataset):
    """
    PyTorch Dataset for Attention-based prediction.
    
    Includes news mask for attention mechanism to ignore days without news.
    """
    ...
    def __init__(
        ...
        self.news_masks = torch.tensor(news_masks, dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        news_mask = self.news_masks[idx]
        label = self.labels[idx]
        
        items = [stock_emb, news_seq, news_mask]
...
def create_data_loaders(
    ...
    # Create datasets
    train_dataset = AttentionPredictionDataset(
        stock_embeddings[train_idx],
        news_sequences[train_idx],
        news_masks[train_idx],
    ...
    val_dataset = AttentionPredictionDataset(
    ...
    test_dataset = AttentionPredictionDataset(
...
if __name__ == "__main__":
    print("Testing Attention Dataset creation...")
    stock_emb, news_seq, news_masks, labels, ticker_idx, tickers, dates = create_combined_dataset(verbose=True)
    print(f"\n✓ Dataset created successfully!")
    print(f"  Stock embeddings: {stock_emb.shape}")
    print(f"  News sequences: {news_seq.shape}")
    print(f"  News masks: {news_masks.shape}")
